...
INSERT into feeds (id, name,url,created_at,updated_at,user_id) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id, name, url, created_at, updated_at, user_id, last_fetched_at
`
...
SELECT id, name, url, created_at, updated_at, user_id, last_fetched_at FROM feeds where id = $1
`
...
const markFieldAsFetched = `-- name: MarkFieldAsFetched :one
...
func (q *Queries) MarkFieldAsFetched(ctx context.Context, id uuid.UUID) (Feed, error) {
	row := q.db.QueryRowContext(ctx, markFieldAsFetched, id)
	var i Feed
	err := row.Scan(
		&i.ID,
		&i.Name,
